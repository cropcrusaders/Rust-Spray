...
// Placeholder algorithm functions (to be implemented)
fn exg(image: &Mat) -> Mat {
    // Placeholder: Replace with actual exg implementation
    image.clone()
}

fn exgr(image: &Mat) -> Mat {
    // Placeholder: Replace with actual exgr implementation
    image.clone()
}

fn maxg(image: &Mat) -> Mat {
    // Placeholder: Replace with actual maxg implementation
    image.clone()
}

fn exg_standardised(image: &Mat) -> Mat {
    // Placeholder: Replace with actual exg_standardised implementation
    image.clone()
}

fn exg_standardised_hue(
    image: &Mat,
    hue_min: i32,
    hue_max: i32,
    brightness_min: i32,
    ...
    // Placeholder: Replace with actual exhsv implementation
...
fn hsv(
    ...
    // Placeholder: Replace with actual hsv implementation
    Ok((image.clone(), true))
}

fn gndvi(image: &Mat) -> Mat {
    // Placeholder: Replace with actual gndvi implementation
    image.clone()
}
...
impl GreenOnBrown {
    pub fn new(algorithm: &str) -> Result<Self> {
        ...
        algorithms.insert("nexg".to_string(), exg_standardised as AlgorithmFn);
        algorithms.insert("gndvi".to_string(), gndvi as AlgorithmFn);

        let mut algorithms_with_params = HashMap::new();
        algorithms_with_params.insert("exhsv".to_string(), exg_standardised_hue as AlgorithmFnWithParams);
    ...
    pub fn inference(
        ...
        if let Some(func) = self.algorithms_with_params.get(algorithm) {
            ...
            let func = self.algorithms.get(algorithm).unwrap_or(&exg_standardised);
            output = func(image);
        }
        ...
        if !threshed_already {
            ...
            // Morphological closing for pre-thresholded output
            imgproc::morphology_ex(
                &output,
                &mut output,
                imgproc::MORPH_CLOSE,
        ...
        // Filter contours and compute boxes and centers
        ...
        let mut image_out = if show_display {
